...
public class PushNotificationReceiver extends BroadcastReceiver {
  private static final String TAG = String.format("%s.%s", Constants.APPBOY_LOG_TAG_PREFIX, PushNotificationReceiver.class.getName());
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if (pushReceivedAction.equals(action)) {
      ...
      if (intent.getBooleanExtra(Constants.APPBOY_ACTION_IS_CUSTOM_ACTION_KEY, false)) {
        Toast.makeText(context, "You clicked a custom action!", Toast.LENGTH_LONG).show();
      } else {
